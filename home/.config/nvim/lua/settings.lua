...
do
    local number = true -- Numbers on left side
...
do
    local swapfile = false -- Don't use swap files
...
do
    local cursorline = true -- Highlight current cursor line
...
-- Tabs and whitespace
do
    local wrap = true -- Wrap lines
    set_global.wrap = true
    set_window.wrap = true
end

do
    local tabstop = 2 -- Tabs are always 2 spaces
    set_global.tabstop = tabstop
    set_buffer.tabstop = tabstop
end

do
    local autoindent = true -- Copy indent from current line when starting a new line
    set_global.autoindent = autoindent
    set_buffer.autoindent = autoindent
end

do
    local softtabstop = 2 -- Number of spaces that a <Tab> counts for while performing editing operations
    set_global.softtabstop = softtabstop
    set_buffer.softtabstop = softtabstop
end

do
    local expandtab = true -- Expand tabs into spaces
    set_global.expandtab = expandtab
    set_buffer.expandtab = expandtab
end

do
    local shiftwidth = vim.o.tabstop -- Reindent with 2 spaces (using <<)
...
do
    local smartindent = true -- Autoindenting when starting a new line
...
do
    local linebreak = true -- Wrap long lines at a character in 'breakat' rather than at the last character that fits on the screen
